...
if [[ ${operation} == "apply" ]] ; then
  ...
  do
    ...
    sed -e "s/\${nested_esxi_root_password}/${NESTED_ESXI_PASSWORD}/" \
        -e "s/\${ip_mgmt}/$(echo ${ips} | jq -r .[$(expr ${esxi} - 1)])/" \
  ...
  do
    count=1
    until $(curl --output /dev/null --silent --head -k https://$(echo ${ips} | jq -r .[$(expr ${esxi} - 1)]))
    do
      echo "Attempt ${count}: Waiting for ESXi host ${esxi} at https://$(echo ${ips} | jq -r .[$(expr ${esxi} - 1)]) to be reachable..."
      sleep 30
      count=$((count+1))
      if [[ "${count}" -eq 30 ]]; then
        echo "ERROR: Unable to connect to ESXi host ${esxi} at https://$(echo ${ips} | jq -r .[$(expr ${esxi} - 1)]) after ${count} Attempts"
        exit 1
      fi
    done
    if [ -z "${slack_webhook_url}" ] ; then echo "ignoring slack update" ; else curl -X POST -H 'Content-type: application/json' --data '{"text":"'$(date "+%Y-%m-%d,%H:%M:%S")', nested-vcf: nested ESXi '${esxi}' configured and reachable"}' ${slack_webhook_url} >/dev/null 2>&1; fi
  done
  for esxi in $(seq 1 $(echo ${ips} | jq -c -r '. | length'))
  do
    ...
    if [ -z "${slack_webhook_url}" ] ; then echo "ignoring slack update" ; else curl -X POST -H 'Content-type: application/json' --data '{"text":"'$(date "+%Y-%m-%d,%H:%M:%S")', nested-vcf: nested ESXi '${esxi}' SSL cert renewed"}' ${slack_webhook_url} >/dev/null 2>&1; fi
  done
fi
#
